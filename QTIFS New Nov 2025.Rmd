...
test <- q %>% dplyr::group_by(as.factor(recommendation_scale)) %>% dplyr::summarize(n())
sum(test$`n()`) # 165
print(test)
...
q_collect <- q %>% 
  ...
  ungroup() %>% 
  mutate(recommendation_categories = fct_relevel(recommendation_categories, c("Climate", "Protocols", "Training",  "Accessibility")),
...
# relevel factors for consistent ordering in plots
q_traverse <- q %>% 
  mutate(recommendation_categories = fct_relevel(recommendation_categories, c("Climate", "Protocols", "Training",  "Accessibility")),
         recommendation_scale = fct_relevel(recommendation_scale, c(
           "Outside Institution",
...
q_heatmap <- q %>%
  ...
  mutate(
    recommendation_categories = fct_relevel(recommendation_categories, c("Climate", "Protocols", "Training",  "Accessibility")),
    recommendation_scale = fct_relevel(recommendation_scale, c(
      "Outside Institution", "Institution", "Department", "Lab", "PI", "Individual"
    ))
  )
...
q_responsibility <- q %>%
  select(article, recommendation_categories, recommendation_scale) %>%
  mutate(recommendation_scale = fct_relevel(recommendation_scale, c(
    "Individual",
    "PI",
    "Lab", 
    "Department",
